...
export default function LoginForm({ appName = "Booking Agent" }) {
    ...
    <main className="grid min-h-screen lg:grid-cols-2">
      <div className="relative hidden overflow-hidden bg-ink lg:flex lg:flex-col lg:justify-between lg:p-12">
        <div className="flex items-center gap-2.5 text-white">
          <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-brand text-sm font-bold">{appName.charAt(0)}</div>
          <span className="text-lg font-semibold tracking-tight">{appName}</span>
        </div>
        <div>
          <h2 className="max-w-md text-3xl font-semibold leading-tight text-white">Every WhatsApp booking, in one calm place.</h2>
          <p className="mt-4 max-w-sm text-sm leading-relaxed text-white/60">Your AI receptionist books appointments around the clock. Sign in to see who's coming in and when.</p>
        </div>
      ...
      <div className="flex items-center justify-center px-6 py-16">
        <div className="w-full max-w-sm">
          <div className="mb-8 lg:hidden">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-brand text-white"><span className="text-lg font-bold">{appName.charAt(0)}</span></div>
          </div>
          ...
          <div className="mt-8 space-y-4">
            ...
            <button onClick={handleSignIn} disabled={loading} className="btn-brand w-full">{loading ? "Signing in…" : "Sign in"}</button>
          </div>
          <p className="mt-6 text-center text-sm text-muted">New here? <Link href="/signup" className="font-medium text-brand hover:text-brand-dark">Create an account</Link></p>
        </div>
      </div>
    </main>
  );
